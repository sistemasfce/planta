...
class ci_modificar_asignacion_edicion extends planta_ci
{
    ...
    function conf__cuadro_des(planta_ei_cuadro $cuadro)
    {
        ...
        foreach ($datos as $dat) {
            $fila = $dat;
            if ($fila['estado'] == comunes::estado_historico)
                continue;            
            $fila['resolucion_desc'] = $dat['resolucion']. '/'.$dat['resolucion_anio']. ' '.$dat['resolucion_tipo_desc'];

            if (($fila['designacion_tipo'] == comunes::desig_alta or $fila['designacion_tipo'] == comunes::desig_modifica)and $fila['designacion'] != null and ($fila['estado'] == comunes::estado_activo or $fila['estado'] == comunes::estado_con_licencia_p)) {
    ...
    function conf_evt__cuadro_des__seleccion(toba_evento_usuario $evento, $fila)
    {
        ...
        if ($datos[$fila]['designacion_tipo'] != comunes::desig_alta and $datos[$fila]['designacion_tipo'] != comunes::desig_modifica) {
                $evento->anular(); 
        } else {
            if ($datos[$fila]['estado'] == comunes::estado_historico or $datos[$fila]['estado'] == comunes::estado_con_licencia_t) {
                $evento->anular(); 
            }
            else {
                $evento->mostrar();  
