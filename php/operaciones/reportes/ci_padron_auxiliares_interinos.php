<?php
class ci_padron_auxiliares_interinos extends planta_ci
{
	protected $s__filtro;
	...
	function conf__cuadro(planta_ei_cuadro $cuadro)
	{
		...
		if ($where != '1=1') {
			...
			$datos = toba::consulta_php('co_designaciones')->get_padron_auxiliares_interinos($where);
	...
	{    
		$report->set_nombre_archivo('Padron de auxiliares de docencia graduados interinos');
		$path_toba = toba::proyecto()->get_path().'/exportaciones/jasper/';
		$path = $path_toba.'padron_profesores.jasper';
		$report->set_path_reporte($path);

		$report->set_parametro('titulo', 'S', 'Padrón de auxiliares de docencia graduados interinos');        
